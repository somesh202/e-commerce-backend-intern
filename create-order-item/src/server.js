...
const execute = async (variables, operation, reqHeaders) => {
  const fetchResponse = await fetch("<your_GraphQL_endpoint", {
    method: "POST",
    headers:
      {
        ...reqHeaders,
...
app.post("/createOrderItem", async (req, res) => {
  ...
  const { data: orderExists, errors: orderExistsErr } = await execute(
    { user_id },
    USER_ORDER_EXISTS,
  ...
  if (
    orderExists.users[0].orders.length === 0 ||
    orderExists.users[0].orders[0].ordered === true
  ) {
    const { data: createOrder, errors: createOrderErr } = await execute(
      {
        city: "",
        country: "",
        house_number: 0,
  ...
  const { data: createOrder, errors: createOrderErr } = await execute(
    { product_id, quantity, order_id: latest_order },
    CREATE_ORDER_ITEM,
    req.body.session_variables
  );
